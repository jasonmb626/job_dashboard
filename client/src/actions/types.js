//Auth
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAILURE = 'LOGIN_FAILURE';
export const AUTH_ERROR = 'AUTH_ERROR';
export const USER_LOADED = 'USER_LOADED';
export const LOGOUT = 'LOGOUT';
export const SET_FIELD = 'SET_FIELD';

//Company
export const CLEAR_POSSIBLE_COMPANIES = 'CLEAR_POSSIBLE_COMPANIES';
export const ADD_POSSIBLE_COMPANIES = 'ADD_POSSIBLE_COMPANIES';

//Jobs
export const ADD_JOB = 'ADD_JOB';
export const CLEAR_JOB = 'CLEAR_JOB';
export const ADD_ACTION = 'ADD_ACTION';
export const ADD_HIRING_MANAGER = 'ADD_HIRING_MANAGER';
export const EDIT_JOB = 'EDIT_JOB';
export const GET_JOBS = 'GET_JOBS';
export const GET_JOB = 'GET_JOB';
export const GET_TEMPLATES = 'GET_TEMPLATES';
export const JOBS_LOADING = 'JOBS_LOADING';
export const JOB_LOADING = 'JOB_LOADING';
export const CLEAR_ACTION = 'CLEAR_ACTION';
export const CLEAR_HIRING_MANAGER = 'CLEAR_HIRING_MANAGER';
